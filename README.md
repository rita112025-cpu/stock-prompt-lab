...
## 使用方式
1. 進入「持股表」，點「匯入持股 CSV」。
2. 預覽後選「取代」或「加入」。取代會替換所有帳戶；加入不會自動去除重複部位。
3. 編輯股數、成本均價、現價後，總覽會同步計算。
4. 複製組合健檢 Prompt 時，會附上目前帳戶篩選的持股與總額。
5. 離開或重新整理前下載 CSV 備份；本站僅將資料保留於本次開啟的記憶體，不會自動上傳或永久保存。

CSV 必填欄位：代號、名稱、股數、成本均價、現價；帳戶、因子可省略，預設「未分類」（因子未填時 00 開頭代號視為 ETF），因子也可在持股表直接修改。支援 UTF-8 與 Big5。代號請以文字欄位保存，避免 Excel 刪掉前導 0。
