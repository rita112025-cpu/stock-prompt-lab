...
CSV 必填欄位：代號、名稱、股數、成本均價、現價；帳戶、因子可省略。缺少帳戶欄時，匯入預覽會要求指定帳戶，不會依目前篩選自動帶入；因子未填時 00 開頭代號視為 ETF，其餘為「未分類」，也可在持股表直接修改。支援 UTF-8 與 Big5。代號請以文字欄位保存，避免 Excel 刪掉前導 0；代號和現有持股只差開頭的 0 時，匯入會被阻止。
